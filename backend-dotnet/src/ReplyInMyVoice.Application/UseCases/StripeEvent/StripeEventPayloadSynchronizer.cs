    ...
    IUnitOfWork unitOfWork)
{
        ...
        RewriteCredit credit)
    {
        ...
        // amount_refunded is Stripe's CUMULATIVE total for the charge and `remaining` is recomputed from
        // the preserved OriginalAmountGranted, so repeated/partial charge.refunded events converge to the
        // same absolute result (idempotent, no compounding) — hence no per-refund-object dedup is needed.
