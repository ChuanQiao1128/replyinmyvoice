    ...
    TimeSpan? cleanupInterval = null) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (await timer.WaitForNextTickAsync(stoppingToken))
        {
            ...
            // Register the in-flight gate BEFORE invoking the handler (which can signal progress on its
            // first synchronous run) so StopAsync deterministically observes and drains this iteration —
            // no window where _inFlightIteration is still null while an iteration is already running.
