...
public sealed class RewriteJobUseCaseTests
{
    ...
    {
        // Pins the property that makes duplicate Service Bus / outbox delivery safe (audit P0-1): a second
        // dispatch of an already-finalized attempt must be a no-op — the rewrite engine is NOT called again
        // (no duplicate processing cost) and quota/cost are not double-counted. A fresh worker scope
        // (new DbContext) processes the redelivery, mirroring production.
        await using var fixture = await DbFixture.CreateAsync();
        var user = await fixture.CreateUserAsync();
        var now = DateTimeOffset.UtcNow;
