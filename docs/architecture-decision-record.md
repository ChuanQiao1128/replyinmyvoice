...
- **Realization decision (added 2026-05-24):** the analytics plane is being built **local-first as a Kafka/K8s/Docker ops sidecar** for a personal ops dashboard + portfolio value — see `docs/ops-analytics-platform-spec.md`. It **preserves every principle here** (async seam, Python off the hot path, separate analytics store) and **diverges only on transport/host** (Kafka-compatible broker + Python consumers + Kubernetes instead of Service Bus + Function App). The Azure-serverless path above **remains the cheapest cloud-production option** and the recommendation if/when the plane graduates to always-on cloud. Managed always-on infra (AKS / Confluent / Event Hubs) stays behind the §6 approval gate (`AZURE_ALLOW_PAID_RESOURCES` / `AZURE_BUDGET_LIMIT`).
