...
| `DispatchWebhookDeliveries` | `WebhookDispatcherTimerFunction` | `*/30 * * * * *` | Posts due API result webhook deliveries and records delivered, retry, or failed state without blocking rewrite completion. | `backend-dotnet/src/ReplyInMyVoice.Functions/Functions/WebhookDispatcherTimerFunction.cs` |
