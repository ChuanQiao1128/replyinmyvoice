...
- **Secrets:** no secret values in source. `API_KEY_PEPPER` is read at runtime by the C# `ApiKeyService` **only** — the Worker/Next layer just forwards the bearer token and never hashes keys — so it is set on the `replyinmyvoice-func-dev` Functions app settings **only**, never in `wrangler.jsonc` `vars` (those are committed plaintext and must never hold a secret). **Set live on prod 2026-06-05** (256-bit hex via `az functionapp config appsettings set`). Caveat: turning a pepper on invalidates any key hashed before it (SHA-256 is one-way; no rehash possible) — pre-pepper keys must be regenerated. Never printed/committed.
