...
function shouldKeepExtractedFact({
  ...
  if (!cleaned || isGenericFact(cleaned)) {
    return false;
  }
...
export function reviewExtractedFacts(
  ...
  if (!facts.recipient_name && facts.people_mentioned.length > 0) {
    facts.recipient_name = facts.people_mentioned[0] ?? "";
  }
